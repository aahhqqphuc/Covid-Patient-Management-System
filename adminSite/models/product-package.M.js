...
module.exports = {
  ...
  add: async (package) => {
    ...
    VALUES(default,'${package.ten_goi}',${package.thoi_gian},${package.muc_gioi_han});`;
    try {
  ...
  updateProduct: async (id, gioi_han) => {
  ...
  deleteProduct: async (id) => {
    const query = `delete from danh_sach_nhu_yeu_pham where id_danh_sach_goi = ${id}`;
    try {
      var rs = await db.any(query);
    } catch (error) {
  ...
  getPackageProducts: async (id) => {
    ...
    const packageProductsQuery = `select distinct on (n.id_nhu_yeu_pham) n.id_nhu_yeu_pham, n.ten_sanpham, d.so_luong, d.gioi_han_san_pham, n.mo_ta, n.gia_tien, h.url
